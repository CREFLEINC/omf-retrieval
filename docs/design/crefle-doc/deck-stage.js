// @ts-nocheck
//
// ⚠️ 왜 typecheck 를 끄는가 (2026-07-17, doc DS 로 이관하며)
//
// 이 파일은 **흡수한 코드**다. crefle_designer 스킬과 CREFLEINC/reports 에서 이미
// 돌고 있던 1,732줄을 그대로 가져와, CSS 문자열 7군데(브랜드 색·폰트)만 고쳤다.
// 로직은 한 줄도 바꾸지 않았다.
//
// tsconfig 는 strict + checkJs 다. 그 아래서 이 파일은 ~50개의 implicit-any /
// possibly-undefined 를 낸다 — 전부 내부 필드와 DOM 조회이고 실제 버그는 없다
// (예: parseInt(getAttribute('width')) 는 null 이면 NaN → `|| DESIGN_W_DEFAULT` 가 받는다).
// 그걸 다 주석 달면 **실제 변경 7줄짜리 이관이 수십 배 큰 diff 가 되어 원본과 대조가
// 불가능해진다.** 흡수의 핵심 이득 — 무엇이 바뀌었는지 한눈에 보이는 것 — 을 잃는다.
//
// 이 파일의 안전망은 타입이 아니라 **tests/e2e/deck.spec.mjs 의 6개 테스트**다.
// 진짜 Chromium 에서 업그레이드·자동스케일 기하·폰트 로드·아이콘 글리프·
// PDF 페이지 크기(1440×810pt)를 실제로 검증한다. jsdom 이었다면 공허했을 것들이다
// (adoptedStyleSheets 대입이 조용히 성공하는 함정 — check-foundation.test.mjs:1-16 참고).
//
// **새로 저작하는 컴포넌트(src/crefle-chart.js 등)에는 이 면제가 없다.** 그것들은
// JSDoc + strict 로 검사한다. 원칙: **저작한 것은 typecheck, 흡수한 것은 e2e.**
//
// 이관 시 고친 7군데 (전부 CSS 문자열, 로직 무변경):
//   · .canvas 배경  #fff        → var(--surface)          (파운데이션이 순수 흰색 금지)
//   · :host 폰트     OS 스택     → var(--font-sans)         (Menlo 류 비결정성 제거)
//   · .count 폰트    OS mono 스택 → var(--font-mono)
//   · .thumb[data-current] 강조 #D97757 → var(--primary)   (Anthropic 코럴 → CREFLE 레드)
//   · .thumb 배지    #D97757     → var(--primary)
//   · .confirm .danger #c96442   → var(--semantic-error)   (파괴적 액션 = semantic error)
//   · .danger:hover  #b5563a + 색 그림자 → var(--semantic-error-strong) + var(--elev-2)
//
/**
...
(() => {
  ...
  const stylesheet = `
    :host {
      ...
      background: #000;
      color: #fff;
      /* OS 의존 스택은 머신마다 다르게 해석된다(발행 PDF 의 Menlo-Bold 참고).
         번들한 서체만 명명한다. 크롬은 인쇄되지 않지만 규칙은 하나로 간다. */
    ...
    .canvas {
      ...
      flex-shrink: 0;
      /* 파운데이션 tokens.css: "순수 #FFFFFF 금지. 약한 마젠타 기 도는
         warm-cool white 사용." 이 캔버스는 크롬이 아니라 브랜드 표면이다. */
    ...
    /* 현재 슬라이드 강조 = 브랜드 강조색. 원본은 #D97757(Anthropic 코럴)이었다. */
    .thumb[data-current] .frame { outline-color: var(--primary); }
    ...
    .confirm .danger:hover { background: var(--semantic-error-strong); }
